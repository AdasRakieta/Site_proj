...
async loadAutomations(){try{const timeoutPromise=new Promise((_,reject)=>{setTimeout(()=>reject(new Error('Timeout ładowania automatyzacji')),5000);});const response=await Promise.race([this.app.fetchData('/api/automations'),timeoutPromise]);}catch(error){console.error('Błąd ładowania automatyzacji:',error);this.renderAutomations([]);return false;}}
onAutomationsUpdate(data){console.log('Aktualizacja automatyzacji:',data);if(Array.isArray(data)){this.automations=data;this.renderAutomations(data);}}
...
automations.forEach((automation,index)=>{const automationElement=this.createAutomationCard(automation,index);list.appendChild(automationElement);});this.setupGlobalClickListener();}
createAutomationCard(automation,index){const actionsContainer=this.app.createElement('div',{class:'automation-actions'});const editBtn=this.app.createElement('button',{class:'edit-automation btn-secondary',textContent:'Edytuj'});const deleteBtn=this.app.createElement('button',{class:'delete-automation btn-secondary',textContent:'Usuń'});const confirmBtn=this.app.createElement('button',{class:'confirm-delete-btn btn-danger hidden',textContent:'Potwierdź'});actionsContainer.appendChild(editBtn);actionsContainer.appendChild(deleteBtn);actionsContainer.appendChild(confirmBtn);const automationElement=this.app.createElement('div',{class:'automation-card','data-index':index},[this.app.createElement('h3',{textContent:automation.name}),this.app.createElement('p',{textContent:`Wyzwalacz:${this.formatTrigger(automation.trigger)}`}),this.app.createElement('p',{textContent:`Akcje:${automation.actions.length}`}),this.app.createElement('div',{class:`status-badge ${automation.enabled?'enabled':'disabled'}`,textContent:automation.enabled?'Aktywna':'Nieaktywna'}),actionsContainer]);editBtn.addEventListener('click',(e)=>{e.stopPropagation();this.showAutomationForm(automation,index);});deleteBtn.addEventListener('click',(e)=>{e.stopPropagation();deleteBtn.classList.add('hidden');confirmBtn.classList.remove('hidden');setTimeout(()=>{if(!confirmBtn.classList.contains('hidden')){confirmBtn.classList.add('hidden');deleteBtn.classList.remove('hidden');}},5000);});confirmBtn.addEventListener('click',async(e)=>{e.stopPropagation();await this.deleteAutomation(index);});return automationElement;}
setupGlobalClickListener(){this.globalClickListener=(e)=>{if(!e.target.closest('.delete-btn-container')){document.querySelectorAll('.delete-btn-container').forEach(container=>{container.querySelector('.delete-automation').classList.remove('hidden');container.querySelector('.confirm-delete-btn').classList.add('hidden');});}};document.addEventListener('click',this.globalClickListener);}
...
populateActions(actions=[]){const container=document.getElementById('actions-container');if(!container)return;container.innerHTML='';actions.forEach(action=>{this.addActionToForm(action);});}
addActionToForm(existingAction=null){const container=document.getElementById('actions-container');if(!container)return;const actionIndex=container.children.length;const actionElement=this.app.createElement('div',{class:'action','data-index':actionIndex});const typeSelect=this.app.createElement('select',{class:'action-type',name:`actions[${actionIndex}][type]`},[this.app.createElement('option',{value:'device',textContent:'Urządzenie'}),this.app.createElement('option',{value:'notification',textContent:'Powiadomienie'})]);const paramsContainer=this.app.createElement('div',{class:'action-params'});const removeButton=this.app.createElement('button',{class:'remove-action',type:'button',textContent:'Usuń'});actionElement.appendChild(typeSelect);actionElement.appendChild(paramsContainer);actionElement.appendChild(removeButton);container.appendChild(actionElement);if(existingAction){typeSelect.value=existingAction.type;}
this.updateActionParams(actionElement,existingAction);typeSelect.addEventListener('change',()=>{this.updateActionParams(actionElement);});removeButton.addEventListener('click',()=>{actionElement.remove();this.renumberActions();});}
renumberActions(){const container=document.getElementById('actions-container');if(!container)return;Array.from(container.children).forEach((actionElement,index)=>{actionElement.dataset.index=index;const typeSelect=actionElement.querySelector('.action-type');if(typeSelect){typeSelect.name=`actions[${index}][type]`;}
...
getFormData(){const formData={name:document.getElementById('automation-name').value,trigger:{type:document.getElementById('trigger-type').value},actions:[],enabled:document.getElementById('automation-enabled').checked};switch(formData.trigger.type){case'time':formData.trigger.time=document.getElementById('trigger-time').value;formData.trigger.days=Array.from(document.querySelectorAll('.trigger-day:checked')).map(cb=>cb.value);break;case'device':formData.trigger.device=document.getElementById('trigger-device').value;formData.trigger.state=document.getElementById('trigger-device-state').value;break;case'sensor':formData.trigger.sensor=document.getElementById('trigger-sensor').value;formData.trigger.value=parseFloat(document.getElementById('trigger-sensor-value').value);formData.trigger.condition=document.getElementById('trigger-sensor-condition').value;break;}
document.querySelectorAll('#actions-container .action').forEach(actionElement=>{const actionType=actionElement.querySelector('.action-type').value;const action={type:actionType};switch(actionType){case'device':action.device=actionElement.querySelector('.action-device').value;action.state=actionElement.querySelector('.action-device-state').value;break;case'notification':action.message=actionElement.querySelector('.action-notification-message').value;break;}
