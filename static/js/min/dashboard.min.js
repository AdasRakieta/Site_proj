...
async function addUser(){const form=document.querySelector('.user-form form');if(!form)return;const formData=new FormData(form);const username=formData.get('username')?.trim();const email=formData.get('email')?.trim();const password=formData.get('password');const role=formData.get('role');if(!username||username.length<3){showNotification('Nazwa użytkownika musi mieć co najmniej 3 znaki','error');return;}
if(!email||!email.includes('@')){showNotification('Podaj poprawny adres email','error');return;}
if(!password||password.length<6){showNotification('Hasło musi mieć co najmniej 6 znaków','error');return;}
try{const response=await fetch('/api/users',{method:'POST',headers:{'Content-Type':'application/json','X-CSRFToken':getCSRFToken()},body:JSON.stringify({username,email,password,role})});const data=await response.json();if(!response.ok)throw new Error(data.message||'Nieznany błąd');showNotification('Użytkownik został dodany pomyślnie','success');form.reset();loadUsers();}catch(error){showNotification(`Nie udało się dodać użytkownika:${error.message}`,'error');}}
...
function initDashboardPage(){if(window.sessionRole==='admin'){loadUsers();initNotificationSettings();fillNotificationUserSelect();}}
...
document.addEventListener('DOMContentLoaded',()=>{initDashboardPage();const userForm=document.querySelector('.user-form form');if(userForm){userForm.addEventListener('submit',async(e)=>{e.preventDefault();await addUser();});}
const addBtn=document.getElementById('addNotificationRecipientBtn');if(addBtn){addBtn.addEventListener('click',addNotificationRecipient);}});