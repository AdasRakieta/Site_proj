console.log('controls.js loaded');function getCSRFToken(){if(window.getCSRFToken&&window.getCSRFToken!==getCSRFToken){return window.getCSRFToken();}
...
if(thermostatsOrder.length>0){fetch('/api/temperature_controls/order',{method:'POST',headers:{'Content-Type':'application/json','X-CSRFToken':getCSRFToken()},body:JSON.stringify({room:room,room_id:roomId,order:thermostatsOrder})}).then(r=>r.json()).then(data=>console.log('Thermostats order updated:',data));}}
...
const endpoint=type==='light'?'/api/buttons':'/api/temperature_controls';fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json','X-CSRFToken':getCSRFToken()},body:JSON.stringify({name,room:roomName,room_id:roomId})}).then(r=>r.json()).then(()=>window.loadKanban());};window.deleteDevice=function(device){const endpoint=device.type==='light'?`/api/buttons/${device.id}`:`/api/temperature_controls/${device.id}`;fetch(endpoint,{method:'DELETE',headers:{'X-CSRFToken':getCSRFToken()}}).then(r=>r.json()).then(data=>{if(data.status==='success'){if(window.showNotification)window.showNotification('Urządzenie usunięte!','success');window.loadKanban();}
else{if(window.showNotification)window.showNotification('Błąd usuwania urządzenia: '+(data.message||'Nieznany błąd'),'error');}});};function startEditDeviceKanban(device,li){li.innerHTML=`<input type='text'class='kanban-edit-input'value='${device.name}'/><div class='kanban-edit-btns'><button class='kanban-save-btn'>✔</button><button class='kanban-cancel-btn'>✖</button></div>`;const input=li.querySelector('input');const btns=li.querySelector('.kanban-edit-btns');const saveBtn=li.querySelector('.kanban-save-btn');const cancelBtn=li.querySelector('.kanban-cancel-btn');function exitEditMode(newName){window.loadKanban();}
saveBtn.onclick=()=>{const newName=input.value.trim();if(!newName||newName===device.name)return exitEditMode(device.name);const endpoint=device.type==='light'?`/api/buttons/${device.id}`:`/api/temperature_controls/${device.id}`;fetch(endpoint,{method:'PUT',headers:{'Content-Type':'application/json','X-CSRFToken':getCSRFToken()},body:JSON.stringify({name:newName})}).then(response=>{if(!response.ok)throw new Error(`HTTP ${response.status}:${response.statusText}`);return response.json();}).then(data=>{if(data.status==='success'){if(window.showNotification)window.showNotification('Nazwa urządzenia zaktualizowana!','success');}else{if(window.showNotification)window.showNotification('Błąd podczas aktualizacji nazwy: '+(data.message||'Nieznany błąd'),'error');}
exitEditMode(newName);}).catch(error=>{if(window.showNotification)window.showNotification('Błąd sieci podczas aktualizacji nazwy','error');exitEditMode(device.name);});};cancelBtn.onclick=()=>{exitEditMode(device.name);};input.onkeydown=(e)=>{if(e.key==='Enter')saveBtn.onclick();if(e.key==='Escape')cancelBtn.onclick();};}
