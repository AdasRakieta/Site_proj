document.addEventListener('DOMContentLoaded', () => {
    ...
    const form1 = document.getElementById('registerProfileForm');
    const form2 = document.getElementById('verificationForm');
    ...
    function showMessage(message, type = 'error') {
        ...
        if (!msgDiv) {
            ...
            // Dodaj wiadomość do aktywnego kroku
            const activeStep = step1.style.display === 'none' ? step2 : step1;
            const activeForm = step1.style.display === 'none' ? form2 : form1;
    ...
    function showStep2() {
        ...
        // Czyść poprzednie wiadomości
        const oldMsg = document.getElementById('registerMessage');
        if (oldMsg) oldMsg.remove();
        // Ustaw focus na pole kodu
    ...
    form1.addEventListener('submit', async (e) => {
        ...
        // Zapisz dane do wysłania w kolejnym kroku
        registrationData = { username, password, email };
        ...
        try {
            ...
            if (response.ok && data.status === 'verification_sent') {
                ...
                showMessage(data.message || 'Wystąpił błąd podczas wysyłania kodu.');
            }
        } catch (error) {
    ...
    form2.addEventListener('submit', async (e) => {
        ...
        if (!verificationCode || verificationCode.length !== 6) {
            showMessage('Wprowadź 6-cyfrowy kod weryfikacyjny.');
            return;
        }

        if (!/^\d{6}$/.test(verificationCode)) {
            showMessage('Kod weryfikacyjny musi składać się z 6 cyfr.');
            return;
        }
    ...
    // Obsługa przycisku "Wróć" (do logowania)
    ...
    // Formatowanie kodu weryfikacyjnego (tylko cyfry)
