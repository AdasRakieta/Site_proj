...
setTimeout(()=>{window.location.href='/login';},1500);}else{showMessage(data.message||'Wystąpił błąd podczas weryfikacji.');}}catch(error){showMessage('Błąd połączenia z serwerem.');}});backToStep1Btn.addEventListener('click',(e)=>{e.preventDefault();showStep1();});const returnBtns=document.querySelectorAll('.return-button');returnBtns.forEach(btn=>{if(btn.id!=='backToStep1'){btn.addEventListener('click',(e)=>{e.preventDefault();window.location.href='/login';});}});const verificationInput=document.getElementById('verificationCode');if(verificationInput){verificationInput.addEventListener('input',(e)=>{e.target.value=e.target.value.replace(/\D/g,'');});}});