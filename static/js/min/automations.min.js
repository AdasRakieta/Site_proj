...
normalizeEnabledValue(value,defaultValue=false){if(value===undefined||value===null){return defaultValue;}
if(typeof value==='string'){const normalized=value.trim().toLowerCase();if(['false','0','no','off'].includes(normalized)){return false;}
...
async fetchInitialData(){try{const response=await this.app.fetchData('/api/devices');let devices=[];if(response&&Array.isArray(response.data)){devices=response.data;}else if(Array.isArray(response)){devices=response;}
this.buttons=devices;console.log(`[AUTOMATIONS]Loaded ${devices.length}devices for automations`);return devices.length>0;}catch(error){console.error('Błąd ładowania urządzeń:',error);return false;}}
...
showAutomationForm(automation=null,index=null){const formContainer=document.getElementById('automation-form-container')||this.app.createElement('div',{id:'automation-form-container'});const isEnabled=this.normalizeEnabledValue(automation?.enabled,automation===null);formContainer.innerHTML=`<h3>${automation?'Edytuj automatyzację':'Nowa automatyzacja'}</h3><form id="automation-form"><div class="form-group"><label for="automation-name">Nazwa:</label><input type="text"id="automation-name"required
...
addActionToForm(formContainer,existingAction=null){const container=formContainer.querySelector('#actions-container');if(!container)return;const actionIndex=container.children.length;const actionElement=this.app.createElement('div',{class:'action','data-index':actionIndex});const typeSelect=this.app.createElement('select',{class:'action-type',name:`actions[${actionIndex}][type]`},[this.app.createElement('option',{value:'device',textContent:'Urządzenie (światła)'}),this.app.createElement('option',{value:'thermostat_control',textContent:'Termostat (włącz/wyłącz)'}),this.app.createElement('option',{value:'set_temperature',textContent:'Ustaw temperaturę'}),this.app.createElement('option',{value:'notification',textContent:'Powiadomienie'})]);const actionMain=this.app.createElement('div',{class:'action-main'});const paramsContainer=this.app.createElement('div',{class:'action-params'});const removeButton=this.app.createElement('button',{class:'remove-action',type:'button',textContent:'Usuń'});actionMain.appendChild(typeSelect);actionMain.appendChild(paramsContainer);actionElement.appendChild(actionMain);actionElement.appendChild(removeButton);container.appendChild(actionElement);if(existingAction){typeSelect.value=existingAction.type;}
...
updateActionParams(actionElement,existingAction=null){const paramsContainer=actionElement.querySelector('.action-params');if(!paramsContainer)return;const actionType=actionElement.querySelector('.action-type').value;const actionIndex=actionElement.dataset.index||0;actionElement.dataset.actionType=actionType;paramsContainer.innerHTML='';switch(actionType){case'device':if(!this.buttons||this.buttons.length===0){paramsContainer.textContent='Brak dostępnych urządzeń';return;}
const lights=this.buttons.filter(btn=>btn.type!=='temperature_control');if(lights.length===0){paramsContainer.textContent='Brak dostępnych świateł';return;}
const deviceSelect=this.app.createElement('select',{class:'action-device',name:`actions[${actionIndex}][device]`,required:true});lights.forEach(button=>{deviceSelect.appendChild(this.app.createElement('option',{value:`${button.room}_${button.name}`,textContent:`${button.room}-${button.name}`,selected:existingAction?.device===`${button.room}_${button.name}`}));});const stateSelect=this.app.createElement('select',{class:'action-device-state',name:`actions[${actionIndex}][state]`,required:true},[this.app.createElement('option',{value:'on',textContent:'Włącz',selected:existingAction?.state==='on'}),this.app.createElement('option',{value:'off',textContent:'Wyłącz',selected:existingAction?.state==='off'}),this.app.createElement('option',{value:'toggle',textContent:'Przełącz',selected:existingAction?.state==='toggle'})]);paramsContainer.appendChild(deviceSelect);paramsContainer.appendChild(stateSelect);break;case'thermostat_control':if(!this.buttons||this.buttons.length===0){paramsContainer.textContent='Brak dostępnych termostatów';return;}
const thermostatControlList=this.buttons.filter(btn=>btn.type==='temperature_control');if(thermostatControlList.length===0){paramsContainer.textContent='Brak dostępnych termostatów';return;}
const thermostatControlSelect=this.app.createElement('select',{class:'action-thermostat-control',name:`actions[${actionIndex}][device]`,required:true});thermostatControlList.forEach(th=>{thermostatControlSelect.appendChild(this.app.createElement('option',{value:`${th.room}_${th.name}`,textContent:`${th.room}-${th.name}`,selected:existingAction?.device===`${th.room}_${th.name}`}));});const thermostatStateSelect=this.app.createElement('select',{class:'action-thermostat-state',name:`actions[${actionIndex}][state]`,required:true},[this.app.createElement('option',{value:'on',textContent:'Włącz',selected:existingAction?.state==='on'}),this.app.createElement('option',{value:'off',textContent:'Wyłącz',selected:existingAction?.state==='off'}),this.app.createElement('option',{value:'toggle',textContent:'Przełącz',selected:existingAction?.state==='toggle'})]);paramsContainer.appendChild(thermostatControlSelect);paramsContainer.appendChild(thermostatStateSelect);break;case'set_temperature':if(!this.buttons||this.buttons.length===0){paramsContainer.textContent='Brak dostępnych termostatów';return;}
const thermostats=this.buttons.filter(btn=>btn.type==='temperature_control');if(thermostats.length===0){paramsContainer.textContent='Brak dostępnych termostatów';return;}
const thermostatSelect=this.app.createElement('select',{class:'action-thermostat',name:`actions[${actionIndex}][thermostat]`,required:true});thermostats.forEach(th=>{thermostatSelect.appendChild(this.app.createElement('option',{value:`${th.room}_${th.name}`,textContent:`${th.room}-${th.name}`,selected:existingAction?.thermostat===`${th.room}_${th.name}`}));});const tempLabel=this.app.createElement('label',{textContent:'Temperatura (°C):'});const tempInput=this.app.createElement('input',{type:'number',class:'action-temperature-value',name:`actions[${actionIndex}][temperature]`,min:16,max:30,step:0.5,value:existingAction?.temperature||22,required:true,style:'width: auto; height: fit-content;'});paramsContainer.appendChild(thermostatSelect);paramsContainer.appendChild(tempLabel);paramsContainer.appendChild(tempInput);break;case'notification':const messageInput=this.app.createElement('input',{type:'text',class:'action-notification-message',id:`action-notification-message-id`,name:`actions[${actionIndex}][message]`,placeholder:'Treść powiadomienia',value:existingAction?.message||'',required:true});paramsContainer.appendChild(messageInput);break;}}
...
queryAll('#actions-container .action').forEach(actionElement=>{const actionType=actionElement.querySelector('.action-type').value;const action={type:actionType};switch(actionType){case'device':action.device=actionElement.querySelector('.action-device').value;action.state=actionElement.querySelector('.action-device-state').value;break;case'thermostat_control':action.device=actionElement.querySelector('.action-thermostat-control').value;action.state=actionElement.querySelector('.action-thermostat-state').value;break;case'set_temperature':action.thermostat=actionElement.querySelector('.action-thermostat').value;action.temperature=parseFloat(actionElement.querySelector('.action-temperature-value').value);break;case'notification':action.message=actionElement.querySelector('.action-notification-message').value;break;}
