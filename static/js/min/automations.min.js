...
const thermostatSelect=this.app.createElement('select',{class:'action-thermostat',name:`actions[${actionIndex}][thermostat]`,required:true});thermostats.forEach(th=>{thermostatSelect.appendChild(this.app.createElement('option',{value:`${th.room}_${th.name}`,textContent:`${th.room}-${th.name}`,selected:existingAction?.thermostat===`${th.room}_${th.name}`}));});const tempLabel=this.app.createElement('label',{textContent:'Temperatura (°C):'});const tempInput=this.app.createElement('input',{type:'number',class:'action-temperature-value',name:`actions[${actionIndex}][temperature]`,min:16,max:30,step:0.5,value:existingAction?.temperature||22,required:true});paramsContainer.appendChild(thermostatSelect);paramsContainer.appendChild(tempLabel);paramsContainer.appendChild(tempInput);break;case'notification':const messageInput=this.app.createElement('input',{type:'text',class:'action-notification-message',id:`action-notification-message-id`,name:`actions[${actionIndex}][message]`,placeholder:'Treść powiadomienia',value:existingAction?.message||'',required:true});paramsContainer.appendChild(messageInput);break;}}
