...
document.getElementById('homeLatitude').value=lat.toFixed(6);document.getElementById('homeLongitude').value=lon.toFixed(6);document.getElementById('homeCountry').value='Poland';try{const response=await fetch('/api/geocode/reverse',{method:'POST',headers:{'Content-Type':'application/json','X-CSRFToken':window.getCSRFToken?window.getCSRFToken():''},body:JSON.stringify({latitude:lat,longitude:lon})});const result=await response.json();if(result.success&&result.data){const data=result.data;if(data.city){document.getElementById('homeCity').value=data.city;}
